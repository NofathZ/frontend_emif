<template>
  <div class="card-container">
    <router-link
      ...
      v-bind:to="'/' + category.slug + '/' + slug"
    >
      <div class="img-box"></div>
      <p
        class="heading-seven"
        style="text-align: center; padding: 0 10px; margin-top: 19px"
      >
...
export default {
  ...
  methods: {
    getCategory() {
      axios
        .get("api/posts/" + this.recommendation.slug + "/category")
        .then((response) => {
          this.category = response.data;
        });
    },
    getRecom() {
      this.slug = this.recommendation.slug;
    },
  },
  created() {
    this.getCategory();

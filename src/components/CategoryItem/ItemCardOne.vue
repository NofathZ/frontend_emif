<template>
  <div class="container" style="margin-bottom: 88px">
    <div class="row">
      ...
      <div
        class="col-lg-8 px-3 d-flex flex-column justify-content-center"
        style="color: white"
      >
        <h2 class="heading-two">{{ item.title }}</h2>
        <p class="lead-five">
          Dipublikasi pada {{ item.published_at }} oleh {{ user.name }}
        </p>
        <p class="lead-three">
          {{ item.content }} ...
          <router-link
            ...
            v-bind:to="'/' + category.slug + '/' + item.slug"
            >Baca selengkapnya</router-link
          >
        </p>
        <p class="heading-seven">#beasiswa, #unggulan, #beasiswajuli2021</p>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    getUser() {
      ...
      axios.get("api/posts/" + this.item.id + "/user").then((response) => {
        this.user = response.data;
      });
    },
    getCategory() {
      axios
        .get("api/posts/" + this.item.slug + "/category")
        .then((response) => {
